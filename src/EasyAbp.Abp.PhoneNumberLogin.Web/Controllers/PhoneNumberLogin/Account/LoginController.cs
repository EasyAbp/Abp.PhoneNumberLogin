...
namespace EasyAbp.Abp.PhoneNumberLogin.Web.Controllers.PhoneNumberLogin.Account
{
    ...
    public class LoginController : AbpControllerBase
    {
        ...
        private static PhoneNumberLoginResult GetLoginResult(SignInResult result)
        {
            if (result.IsLockedOut)
            {
                return new PhoneNumberLoginResult(PhoneNumberLoginResultType.LockedOut);
            }

            if (result.RequiresTwoFactor)
            {
                return new PhoneNumberLoginResult(PhoneNumberLoginResultType.RequiresTwoFactor);
            }

            if (result.IsNotAllowed)
            {
                return new PhoneNumberLoginResult(PhoneNumberLoginResultType.NotAllowed);
            }
